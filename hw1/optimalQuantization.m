...
%     Method:  Uses the iterative Loyd-Max algorithm for Optimal Quantization as described in class to 
%                   perform quantization. Use the histogram of img8bit as the graylevel probability 
%                   distribution. Use the Loyd-Max algorithm to compute the 2^N+1 Zi values and the 2^N qi values. 
...
hist = histImage(img8bit);

% Initial Z and Q guess - similiar to uniform.
prevZ = ceil(linspace(0,255,2^N + 1 ));
prevQ = (0:(2^N-1)) * (2^(8-N)) + 2^(8-N-1);
nextZ = zeoros(1, 256);
